...
LIST_HEAD(stack);
LIST_HEAD(queue_High);
LIST_HEAD(queue_Middle);
LIST_HEAD(queue_low);

static void add_element_to_queue_High(char *node_element_msg){
	...
	list_add_tail(&(tmp_element->list), &queue_High);
}

static void add_element_to_queue_Middle(char *node_element_msg){
	struct string_node *tmp_element;
	tmp_element = kmalloc(sizeof(struct string_node), GFP_KERNEL);
	strcpy(tmp_element->message, node_element_msg);
	INIT_LIST_HEAD(&tmp_element->list);
	list_add_tail(&(tmp_element->list), &queue_Middle);
}
static void add_element_to_queue_low(char *node_element_msg){
	struct string_node *tmp_element;
	tmp_element = kmalloc(sizeof(struct string_node), GFP_KERNEL);
	strcpy(tmp_element->message, node_element_msg);
	INIT_LIST_HEAD(&tmp_element->list);
	list_add_tail(&(tmp_element->list), &queue_low);
}
...
void myqueue_exit(void)
{
	...
	list_for_each_safe(watch, next, &queue_High)
	{
		tmp_element = list_entry(watch, struct string_node, list);
		list_del(&(tmp_element->list));
		kfree(tmp_element);
	...
	list_for_each_safe(watch, next, &queue_Middle)
	{
		tmp_element = list_entry(watch, struct string_node, list);
		list_del(&(tmp_element->list));
		kfree(tmp_element);
	...
	list_for_each_safe(watch, next, &queue_low)
	{
		tmp_element = list_entry(watch, struct string_node, list);
		list_del(&(tmp_element->list));
		kfree(tmp_element);
...
static long bridge_ioctl(struct file *f, unsigned int cmd, unsigned long arg){
	...
	case BRIDGE_W_HIGH_PRIOR_Q:
	    ...
	    add_element_to_queue_High(message);
        printk(KERN_INFO "Element succesfully added to the queue HIGH\n");
	    break;
	case BRIDGE_W_MIDDLE_PRIOR_Q:
	    raw_copy_from_user(message, (char *)arg, 100);
	    add_element_to_queue_Middle(message);
        printk(KERN_INFO "Element succesfully added to the queue MIDDLE\n");
	    break;
	case BRIDGE_W_LOW_PRIOR_Q:
	    raw_copy_from_user(message, (char *)arg, 100);
	    add_element_to_queue_low(message);
        printk(KERN_INFO "Element succesfully added to the queue LOW\n");
	    break;
	case BRIDGE_R_HIGH_PRIOR_Q:
        tmp_element = list_first_entry(&queue_High, struct string_node, list);
        list_del(&(tmp_element->list));
	    raw_copy_to_user((char *)arg, tmp_element->message, 100);
	    kfree(tmp_element);
	    break;
	case BRIDGE_R_MIDDLE_PRIOR_Q:
        tmp_element = list_first_entry(&queue_Middle, struct string_node, list);
        list_del(&(tmp_element->list));
	    raw_copy_to_user((char *)arg, tmp_element->message, 100);
	    kfree(tmp_element);
	    break;
	case BRIDGE_R_LOW_PRIOR_Q:
        tmp_element = list_first_entry(&queue_low, struct string_node, list);
        list_del(&(tmp_element->list));
	    raw_copy_to_user((char *)arg, tmp_element->message, 100);
	    kfree(tmp_element);
	    break;
